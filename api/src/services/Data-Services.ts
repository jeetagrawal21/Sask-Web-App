...
export async function getData(id: number) {
  ...
  const result = await pool.query(
    ...
    'SELECT "Response Time", "[18_SAQ] In the past 30 days how often have you experienced" FROM "userdata" WHERE "[18_SAQ] In the past 30 days how often have you experienced" IS NOT NULL AND "id" = $1',
    [id]
  );
  await pool.end();
...
export function formatDataForLineChart(data: DataItem[]) {
  ...
  return data.map((item: DataItem) => {
    return {
      ...
      "Cough severity":
        item["[18_SAQ] In the past 30 days how often have you experienced"],
